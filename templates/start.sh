...
if [ -z "$emulator_opts" ]
then
  emulator_opts="-no-accel -screen multi-touch -no-boot-anim -noaudio -nojni -netfast -verbose -camera-back none -camera-front none -skip-adb-auth -snapshot default -no-snapshot-save"
fi
